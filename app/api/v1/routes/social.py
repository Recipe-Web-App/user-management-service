...
async def follow_user(
    ...
    Args:
        user_id: The user's unique identifier (should match authenticated user)
    ...
    """
    if user_id != UUID(authenticated_user_id):
        raise HTTPException(
            status_code=HTTPStatus.UNAUTHORIZED,
            detail="Authenticated user does not match path user_id.",
        )
    return await social_service.follow_user(
        follower_id=user_id, target_user_id=target_user_id
    )
...
@router.get(
    ...
    responses={
        ...
        HTTPStatus.FORBIDDEN: {
            "model": ErrorResponse,
            "description": "Activity is private",
        },
        HTTPStatus.NOT_FOUND: {
            "model": ErrorResponse,
            "description": "User not found",
